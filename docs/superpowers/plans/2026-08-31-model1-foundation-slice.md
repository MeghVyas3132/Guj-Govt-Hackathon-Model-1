...
Add these fixtures to `tests/conftest.py`. The model import is required — `Base.metadata`
is empty until the model modules are imported, so `create_all` would otherwise create no
tables for any test that does not import them itself:
...
In `app/api/v1/routers/cameras.py`, delete the `_STUB_CAMERA` constant and replace **both**
`list_cameras` and `get_camera` — `get_camera` still returns the deleted constant and will
raise `NameError` at request time if you only replace the first. Extract the projection into
a shared `_to_read(row)` helper so the two endpoints cannot drift:
